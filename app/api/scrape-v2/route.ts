/**
 * Unified Scrape API v2
 * Uses Browserless + direct scraping (replaces Scrape.do)
 * 
 * POST /api/scrape-v2
...
import { scrapeCard, scrapeBatch, logScrapeMetrics, getCostBreakdown } from '@/lib/scraping/engine-v2';
...
export async function POST(request: NextRequest) {
  try {
    ...
    console.error('[API /scrape-v2] Error:', error);
    return NextResponse.json(
...
async function handleSingleScrape(cardId: string, specificProvider?: Provider) {
  ...
  // Scrape the card using v2 engine
  ...
  return NextResponse.json({
    ...
    cost: {
      ...
      freeRequests: urls.length - result.metrics.creditsUsed,
      savingsVsAllPaid: `${Math.round((1 - result.metrics.creditsUsed / urls.length) * 100)}%`,
    },
  });
}
...
async function handleBatchScrape(batch: Array<{ cardId: string; provider?: Provider }>) {
  ...
  for (const item of batch) {
    ...
    totalFree += urls.length - result.metrics.creditsUsed;
  ...
  return NextResponse.json({
    ...
    totalCredits,
    totalFree,
    savingsVsAllPaid: `${Math.round((totalFree / (totalCredits + totalFree)) * 100)}%`,
...
async function persistOffers(
  ...
  for (const offer of offers) {
    try {
      ...
      console.error(`[API /scrape-v2] Failed to persist offer for ${cardId}:`, error);
    }
  }
...
export async function GET() {
  ...
  return NextResponse.json({
    ...
    apiKeyConfigured: !!process.env.BROWSERLESS_TOKEN,
    oldApiConfigured: !!process.env.SCRAPE_DO_API_KEY,
    migration: {
      status: 'Use /api/scrape-v2 for new scraper',
      oldEndpoint: '/api/scrape (Scrape.do) - DEPRECATED',
    },
    cost: {
      freeSites: costBreakdown.free,
      paidSites: costBreakdown.paid,
      estimatedSavings: costBreakdown.estimatedSavings,
      note: 'Free sites use direct HTTP fetch. Paid sites use Browserless for Cloudflare bypass.',
    },
  });
}
